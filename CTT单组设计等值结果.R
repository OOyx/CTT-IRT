...
setwd("C:/Users/oyxen/Desktop/等值项目/11.27模拟数据/CTT测试数据/01 单组设计")
...
# 计算科目1下试卷1-5与标准卷的等值结果
for(i in 1:5){
# 统计试卷和标准卷的频数
  test_frq = freqtab(sub1_data[[i]][4],scales = 0:120)
  std_frq = freqtab(sub1_data[[i]][5],,scales = 0:120)
  
# 线性等值
  linear1[[i]]= equate(test_frq, std_frq, type = "linear")

# 等百分位等值
  x_feq = as.data.frame(freqtab(test_frq,scales = 0:120))
  y_feq = as.data.frame(freqtab(std_frq,scales = 0:120))
...
# 计算科目2下试卷1-5与标准卷的等值结果
for(i in 1:5){
  # 统计试卷和标准卷的频数
  test_frq = freqtab(sub2_data[[i]][4],scales = 0:120)
  std_frq = freqtab(sub2_data[[i]][5],,scales = 0:120)
  
  # 线性等值
  linear2[[i]]= equate(test_frq, std_frq, type = "linear")
  ...
  file_name = paste0("科目2试卷",i,".csv")
  write.csv(result,file_name)
}

# 爱德华等值结果(仅选取科目1试卷1)

test_frq = as.data.frame(freqtab(sub1_data[[1]]$Score,scales = 0:120))
std_frq = as.data.frame(freqtab(sub1_data[[1]]$Standard_Score,,scales = 0:120))
